...
class CosyVoice2PreAndPostLayerWeight(Qwen2PreAndPostLayerWeight):
    ...
    def load_hf_weights(self, weights):
        ...
        if (
            self.wte_weight_ is not None
            and self.llm_embedding_weight_ is not None
            and self.speech_embedding_weight_ is not None
        ):
            self.text_llm_audio_emb = torch.cat(
                [self.wte_weight_, self.llm_embedding_weight_, self.speech_embedding_weight_], dim=0
            )
